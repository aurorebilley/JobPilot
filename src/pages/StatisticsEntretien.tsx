...
export default function StatisticsEntretien() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        setEntretienCount(entSnap.size);
        ...
        // Process each entretien
        await Promise.all(entSnap.docs.map(async (entDoc) => {
          const entData = entDoc.data();
          
          // Count status
          if (entData.status === 'en attente') counts.en_attente++;
          else if (entData.status === 'Procédure de recrutement 1') counts.procedure1++;
          else if (entData.status === 'Procédure de recrutement 2') counts.procedure2++;
          else if (entData.status === 'refusé') counts.refuse++;
          else if (entData.status === 'validé') counts.valide++;
          
          // Get linked candidature data
          if (entData.candidatureId) {
            const candRef = doc(db, 'candidatures', entData.candidatureId);
            const candDoc = await getDoc(candRef);
            if (candDoc.exists()) {
              const candData = candDoc.data();
              if (candData.applicationMethod) {
                methodMap.set(candData.applicationMethod, (methodMap.get(candData.applicationMethod) || 0) + 1);
              }
              if (candData.businessSector) {
                sectorMap.set(candData.businessSector, (sectorMap.get(candData.businessSector) || 0) + 1);
              }
            }
          }
        }));
      ...
      <main className="flex-grow w-full py-6 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Statistiques des entretiens</h1>
          </div>
          ...
          <>
            <div className="bg-white shadow-lg rounded-xl p-6">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-4xl font-bold text-gray-900">{entretienCount}</h2>
                  <p className="text-lg text-gray-600">entretiens</p>
                </div>
              </div>
            ...
            <div className="mt-8 grid grid-cols-2 md:grid-cols-3 gap-6">
              <div className="bg-white shadow-lg rounded-xl p-6">
                ...
                <p className="text-4xl font-bold text-[#f2bd64]">{statusCounts.en_attente}</p>
              </div>
              
              <div className="bg-white shadow-lg rounded-xl p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-2">Procédure 1</h3>
                <p className="text-4xl font-bold text-[#f2bd64]">{statusCounts.procedure1}</p>
              </div>
              
              <div className="bg-white shadow-lg rounded-xl p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-2">Procédure 2</h3>
                <p className="text-4xl font-bold text-[#4e8d60]">{statusCounts.procedure2}</p>
              </div>
              
              <div className="bg-white shadow-lg rounded-xl p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-2">Refusés</h3>
                <p className="text-4xl font-bold text-red-500">{statusCounts.refuse}</p>
              </div>
              
              <div className="bg-white shadow-lg rounded-xl p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-2">Validés</h3>
                <p className="text-4xl font-bold text-[#4e8d60]">{statusCounts.valide}</p>
              </div>
            </div>
