/*
  # Set Firebase credentials for check-notifications function

  1. Create http extension if not exists
  2. Create function to set environment variables via HTTP request
  3. Execute the function to apply the configuration
*/

-- Create the http extension if it doesn't exist
CREATE EXTENSION IF NOT EXISTS http WITH SCHEMA extensions;

-- Create a function to set the environment variables
CREATE OR REPLACE FUNCTION set_edge_function_vars() RETURNS void AS $$
BEGIN
  -- Make HTTP request to set environment variables
  PERFORM http((
    'POST',
    'https://jmrzsjqupjubldwxvlec.functions.supabase.co/config',
    ARRAY[
      ('Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImptcnpzanF1cGp1Ymxkd3h2bGVjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQ4MDc2MDMsImV4cCI6MjA2MDM4MzYwM30.VvVCZvAnjXhG8E_4gcw12yA2nHnQJ4RnamKwMLiOdKc')::http_header,
      ('Content-Type', 'application/json')::http_header
    ],
    'application/json',
    json_build_object(
      'name', 'check-notifications',
      'value', json_build_object(
        'FIREBASE_CLIENT_EMAIL', 'firebase-adminsdk-fbsvc@jobpilot-1a04a.iam.gserviceaccount.com',
        'FIREBASE_PRIVATE_KEY', '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
      )
    )::text
  ));
END;
$$ LANGUAGE plpgsql;

-- Execute the function
SELECT set_edge_function_vars();